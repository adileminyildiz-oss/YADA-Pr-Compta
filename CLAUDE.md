...
3. **Module Admin en PLEINE LARGEUR (sans barre latérale)** — cliquer une tuile n'affiche plus la barre latérale des modules : le contenu occupe toute la largeur (`.adm-solo`), avec une petite barre en haut (**‹ Modules** pour revenir à la liste · **← Retour aux dossiers**).
...
**Comment — `yada-addon210` (éditions) :** keyframe `admHubIn` (blur 14px→0, translateY −14px→0) `.72s` + stagger 0/.16/.32/.48 ; `admHubBack()` ajoute `.adm-hub-out` (keyframe `admHubOut`, stagger inversé) puis `admShowToggle(false)` après 1250 ms ; interception `admShow && admModule` → rendu `.adm-solo` (barre `‹ Modules`/`← Retour` + `#adm-content` = `admModuleContent()`), **sans** la `.adm-shell`/`.adm-side` d'addon202 ; branche « Liste des dossiers » (Importer présent, pas de bouton Liste Dossier) → injection `#adm-enter-inline` après Importer. `prefers-reduced-motion` respecté.
